import type {
  ShippingMode,
  ProductCategory
} from "@/app/provider/ProductProvider"
...
export function buildGeminiPayload(p: Params): string {
  ...
  if (p.aiCurrentPrice > 0)
    lines.push(
      `- Precio de venta (hoy): $${p.aiCurrentPrice.toLocaleString("es-CO")}`
    )
  if (p.aiPreviousPrice > 0)
    lines.push(
      `- Precio anterior (tachado): $${p.aiPreviousPrice.toLocaleString("es-CO")}`
    )
  ...
  const currentFmt =
    p.aiCurrentPrice > 0
      ? `$${p.aiCurrentPrice.toLocaleString("es-CO")}`
      : "{precio_actual}"
  ...
  lines.push(
    "Genera un mensaje promocional para WhatsApp con el formato exacto de abajo."
  )
  lines.push(
    "Devuelve el resultado dentro de un bloque de código (``` ```) para que sea fácil de copiar."
  )
  lines.push(
    "Traduce todo al español (conserva nombres de marca y referencias técnicas)."
  )
  lines.push(
...
export function buildAdminPayload(p: AdminParams): string {
  ...
  if (p.productLink) lines.push(`Link: ${p.productLink}`)
  if (p.couponCode) lines.push(`Cupón: ${p.couponCode}`)
  if (p.businessPrice > 0) lines.push(`Precio: ${p.businessPrice} USD`)
