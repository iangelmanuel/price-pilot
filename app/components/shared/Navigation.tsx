...
export function Navigation() {
  ...
  const menuItems = [
    ...
    { href: "/cotizacion-rapida", label: "Cotización rápida", icon: ZapIcon }
  ]
  ...
  return menuItems.map(({ href, label, icon: Icon }) => (
    ...
    >
      <Link
        href={href}
        className={[
          "flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-[15px] font-medium transition-all sm:px-3 sm:py-1 sm:text-[17px] lg:text-[19px]",
          pathname === href
